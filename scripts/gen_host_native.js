...
const functionsToExclude = [
  ...
  // between WASM32 (32-bit pointers in struct) and host (64-bit pointers)
...
await mkdir(join(__dirname, '../src/host/native'), { recursive: true })
const output = generateHostNative()
await writeFile(join(__dirname, '../src/host/native/host_native.c'), output)

console.log('Generated src/host/native/host_native.c')

...
export class Raylib {
  ...
  _setFont(fontSize, fontPtr) {
    const family = fontPtr ? this._fontFamily(fontPtr) : this._defaultFontFamily
  ...
  DrawTextCodepoint(fontPtr, codepoint, posPtr, fontSize, colorPtr) {
    const pos = this._readVec2(posPtr)
    this.DrawText(this._mallocStr(String.fromCodePoint(codepoint)), pos.x, pos.y, fontSize, colorPtr)
  }
  ...
  DrawTextCodepoints(fontPtr, codepointsPtr, codepointCount, posPtr, fontSize, spacing, colorPtr) {
    ...
    const pos = this._readVec2(posPtr)
    this.DrawText(this._mallocStr(chars.join('')), pos.x, pos.y, fontSize, colorPtr)
  }
  ...
  MeasureTextEx(resultPtr, fontPtr, textPtr, fontSize, spacing) {
    this._setFont(fontSize, fontPtr)
    const text = this._readStr(textPtr), lines = text.split('\n')
    let maxW = 0
  ...
  MeasureTextCodepoints(resultPtr, fontPtr, codepointsPtr, codepointCount, fontSize, spacing) {
    this._writeVec2(resultPtr, codepointCount * fontSize * 0.6, fontSize)
  }
  ...
  GetGlyphIndex(fontPtr, codepoint) {
    // Return codepoint as index (for basic ASCII; we don't have real glyph tables)
    return codepoint
  }
  ...
  GetGlyphInfo(resultPtr, fontPtr, codepoint) {
    ...
    const v = this._view
    v.setInt32(resultPtr, codepoint, true)       // value
    v.setInt32(resultPtr + 4, 0, true)           // offsetX
    v.setInt32(resultPtr + 8, 0, true)           // offsetY
    v.setInt32(resultPtr + 12, adv, true)        // advanceX
    this._writeImage(resultPtr + 16, 0, adv, fontSize) // image stub
  }
  ...
  // Use canvas default font for default font id=0
  GetFontDefault(resultPtr) {
    this._writeFont(resultPtr, 0, this._defaultFontSize)
  }
  ...
  _fontFamily(fontPtr) {
    if (!fontPtr) return this._defaultFontFamily
    const id = this._getFontId(fontPtr)
    if (id === 0) return this._defaultFontFamily
    return this._fonts.get(id)?.family || this._defaultFontFamily
  }

  _fontBaseSize(fontPtr) {
    if (!fontPtr) return this._defaultFontSize
    const id = this._getFontId(fontPtr)
    if (id === 0) return this._defaultFontSize
    return this._fonts.get(id)?.baseSize || this._defaultFontSize
  }

...
const defines = api.defines.map(generateDefine).filter(d => d !== null).join('\n');
const filteredFunctions = api.functions.filter(f => !shouldExcludeFunction(f));
const functionImports = filteredFunctions
  .map(generateFunctionDeclaration)
  .join('\n\n');
...
console.log(`Generated carts/c/raycart.h with:`);
console.log(`  - ${api.structs.length} structs`);
console.log(`  - ${api.aliases?.length || 0} type aliases`);
console.log(`  - ${api.enums.length} enums`);
console.log(`  - ${api.defines.filter(d => generateDefine(d) !== null).length} defines`);
console.log(`  - ${filteredFunctions.length} function imports`);

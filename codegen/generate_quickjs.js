...
console.log(`Generated carts/c/js/raycart_bindings.c with:`);
console.log(`  - ${usedStructs.length} struct converters`);
console.log(`  - ${filteredFunctions.length} function wrappers`);
console.log(`  - ${api.defines.filter(d => d.type === 'COLOR').length} color constants`);
console.log(`  - ${api.enums.reduce((sum, e) => sum + e.values.length, 0)} enum values`);

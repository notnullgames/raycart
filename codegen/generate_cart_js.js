...
function generateStructToJs(struct) {
  const lines = [];
  lines.push(
    `static JSValue ${struct.name.toLowerCase()}_to_js(${struct.name} value) {`,
  );
  ...
  for (const field of struct.fields) {
    ...
    if (converter && converter.to) {
      lines.push(
        `  JS_SetPropertyStr(ctx, obj, "${field.name}", ${converter.to}(value.${field.name}));`,
      );
...
function generateStructFromJs(struct) {
  const lines = [];
  lines.push(
    `static ${struct.name} ${struct.name.toLowerCase()}_from_js(JSValue obj) {`,
  );
  ...
  for (const field of struct.fields) {
    ...
    if (converter && converter.from) {
      lines.push(
        `  JSValue ${field.name}_val = JS_GetPropertyStr(ctx, obj, "${field.name}");`,
      );
      lines.push(`  if (!JS_IsUndefined(${field.name}_val)) {`);
      lines.push(
        `    result.${field.name} = ${converter.from}(${field.name}_val);`,
      );
      lines.push(`  }`);
...
function generateJsFunction(func) {
  ...
  lines.push(
    `static JSValue js_${func.name}(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {`,
  );
  ...
  if (func.returnType === "void") {
    ...
    if (returnConverter && returnConverter.to) {
      lines.push(
        `  return ${returnConverter.to}(${func.name}(${callParams.join(", ")}));`,
      );
    } else {
...
function generateExposeFunction(filteredFunctions) {
  ...
  for (const define of colorDefines) {
    lines.push(
      `  JS_SetPropertyStr(ctx, global, "${define.name}", color_to_js(${define.name}));`,
    );
  }
  ...
  for (const enumDef of api.enums) {
    for (const value of enumDef.values) {
      lines.push(
        `  JS_SetPropertyStr(ctx, global, "${value.name}", i32_to_js(${value.name}));`,
      );
    }
  }
  ...
  for (const func of filteredFunctions) {
    ...
    lines.push(
      `  JS_SetPropertyStr(ctx, global, "${func.name}", JS_NewCFunction(ctx, js_${func.name}, "${func.name}", ${paramCount}));`,
    );
  }
...
const filteredFunctions = api.functions.filter(
  (f) => !shouldExcludeFunction(f),
);
...
console.log(
  `  Color constants:   ${api.defines.filter((d) => d.type === "COLOR").length}`,
);
console.log(
  `  Enum values:       ${api.enums.reduce((sum, e) => sum + e.values.length, 0)}`,
);
console.log(
  `  Functions:         ${filteredFunctions.length}/${totalFunctions}`,
);
